...
class CrackedToScrapper(BaseScrapper):
    def __init__(self, kwargs):
        super(CrackedToScrapper, self).__init__(kwargs)
    ...
    def write_paginated_data(self, html_response):
        ...
        pattern = re.compile(r'cracked\.to/(.*)\?page=(\d+)')
    ...
    def get_forum_urls(self, html_response):
        final_urls = list()
        main_forum_urls = html_response.xpath(
            '//div[@class="forum" and @id]'
            '//div[@class="flex-grow"]'
            '//a[@title]/@href')
        sub_forum_urls = html_response.xpath(
            '//div[@class="forum-subforums" and @id]//a[@title]/@href')
        for url in main_forum_urls:
            url = self.base_url + url if self.base_url not in url else url
            if url not in final_urls:
                final_urls.append(url)
        for url in sub_forum_urls:
            url = self.base_url + url if self.base_url not in url else url
            if url not in final_urls:
                final_urls.append(url)
        return final_urls
    ...
    def do_scrape(self):
        print('**************  CrackedTo Scrapper Started  **************\n')
