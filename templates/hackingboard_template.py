...
class HackingBoardParser:
    def __init__(self, parser_name, files, output_folder, folder_path):
        # locale.setlocale(locale.LC_TIME, 'ru_RU.UTF-8')
        self.parser_name = "hackingboard.net"
    ...
    def header_data_extract(self, html_response, template):
        try:
            ...
            if not header:
                return
            if not self.get_comment_id(header[0], 1) == "1":
                return
            title = self.get_title(html_response)
            date = self.get_date(header[0], 1)
    ...
    def get_comment_id(self, tag, index):
        comment_id = ""
