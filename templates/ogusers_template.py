...
class OgUsersParser:
    ...
    def get_date(self, tag):
        date_block = tag.xpath(
            'div//div[@class="inline pb_date smalltext"]/span/@title')
        if not date_block:
            date_block = tag.xpath(
    ...
    def get_title(self, tag):
        title = tag.xpath(
            '//span[@class="showthreadtopbar_size"]/text()'
        )
    ...
    def get_avatar(self, tag):
        avatar_block = tag.xpath(
            'aside//div[@class="postbit-avatar"]/a/img/@src'
        )
        if not avatar_block:
            return ""
    ...
    def get_comment_id(self, tag):
        comment_block = tag.xpath(
            'div//div[@class="float_right postbitnum"]/strong/a/text()')
        if not comment_block:
            return ''
        return comment_block[0].split('#')[-1].replace(',', '').replace('.', '')
