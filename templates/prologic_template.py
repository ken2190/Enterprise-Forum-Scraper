...
class ProLogicParser:
    ...
    def extract_comments(self, html_response):
        ...
        comment_blocks = html_response.xpath(
          '//div[contains(@id, "post_id_")]/div[@class="post_wrap"]'
        )
    ...
    def get_date(self, tag):
        date = tag.xpath(
            'h3/div[@class="post_date"]/abbr/@title'
        )
    ...
    def get_author(self, tag):
        author = tag.xpath(
            'h3//span[@class="author vcard"]/a/span/text()'
        )
        if not author:
            author = tag.xpath(
    ...
    def get_title(self, tag):
        title = tag.xpath(
            '//div[@class="maintitle clear clearfix"]/span/text()'
        )
    ...
    def get_post_text(self, tag):
        post_text_block = tag.xpath(
            'div[@class="post_body"]/div[@itemprop="commentText"]'
            '/descendant::text()[not(ancestor::div[@class="blockquote"]) '
            'and not(ancestor::p[@class="citation"])]'
        )
        post_text = " ".join([
