...
class MalvultParser(BaseTemplate):
    def __init__(self, *args, **kwargs):
        ...
        self.pagination_pattern = re.compile(r".*-(\d+)\.html$")
        ...
        self.date_xpath = './/span[@class="DateTime"]/@title|' \
                          './/abbr[@class="DateTime"]/@title|' \
                          './/abbr[@class="DateTime"]/@data-time'
        self.author_xpath = (
            './/div//div[@class="uix_userTextInner"]/a[@class="username"]/text()'
        )
    ...
    def get_filtered_files(self, files):
        filtered_files = list(
            filter(lambda x: self.thread_name_pattern.search(
                x) is not None, files)
        )
        sorted_files = sorted(
            filtered_files,
            key=lambda x: (
                self.thread_name_pattern.search(x).group(1),
    ...
    def get_post_text(self, tag):
        ...
        post_text = (
            "\n".join([text.strip() for text in post_text if text.strip()])
            if post_text
            else ""
        )

        if not post_text:
            post_text = tag.xpath('div//blockquote[contains(@class,"messageText")]/span/div/text()')
            post_text = (
                "\n".join([text.strip() for text in post_text if text.strip()])
                if post_text
                else ""
