...
class CCCMNParser:
    ...
    def get_date(self, tag):
        date_block = tag.xpath(
            'div//abbr[@itemprop="commentTime"]/text()'
        )
        ...
        try:
            ...
            date_block = tag.xpath(
                'div//abbr[@itemprop="commentTime"]/@title'
            )
    ...
    def get_author(self, tag):
        author = tag.xpath(
            'h3/span[@itemprop="creator name"]/text()'
        )
    ...
    def get_title(self, tag):
        title = tag.xpath(
            '//div[@class="ipsBox_withphoto"]/class/text()'
        )
    ...
    def get_post_text(self, tag):
        post_text_block = tag.xpath(
            'div/div[@itemprop="commentText"]'
            '/descendant::text()['
            'not(ancestor::p[@class="citation"]) and'
            'not(ancestor::div[@class="blockquote"])]'
        )
        post_text = " ".join([
