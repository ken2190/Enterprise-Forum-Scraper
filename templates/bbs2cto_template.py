...
class Bbs2ctoParser:
    ...
    def header_data_extract(self, html_response, template):
        try:
            ...
            if not header:
                return
            if not self.get_comment_id(header[0]) == "0":
                return
    ...
    def get_author(self, tag):
        author = tag.xpath(
            './/div[contains(@class,"readName")]/a//text()'
        )
    ...
    def get_post_text(self, tag):
        post_text_block = tag.xpath(            
            './/div[contains(@class,"tpc_content")]//text()'
        )
    ...
    def get_avatar(self, tag):
        avatar_block = tag.xpath(
            './/a[contains(@class,"userCard")]/img/@src'
        )
        if not avatar_block:
            return ""
