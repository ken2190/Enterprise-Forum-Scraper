...
class MazaParser:
    ...
    def get_date(self, tag):
        date_block = tag.xpath(
            './/span[@class="time"]/preceding-sibling::text()')
        if not date_block:
            return ''
        try:
            date = dateparser.parse(date_block[0])
    ...
    def get_post_text(self, tag):
        post_text_block = tag.xpath(
            './/blockquote[contains(@class,"postcontent restore")]'
