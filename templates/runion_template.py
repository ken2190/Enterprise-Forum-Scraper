...
class RUnionParser:
    ...
    def extract_comments(self, html_response, pagination):
        ...
        comment_blocks = html_response.xpath(
          '//div[contains(@class, "blockpost row")]'
        )
    ...
    def get_date(self, tag):
        date = tag.xpath(
            'h2//span[@class="conr"]/following-sibling::a[1]/text()'
        )
        if not date:
            return ''
        dt_format = "%Y-%m-%d %H ч."
        try:
            date = datetime.datetime.strptime(date[0], dt_format).timestamp()
            return str(date)
        except:
            return ""
    ...
    def get_post_text(self, tag):
        post_text_block = tag.xpath(
            'div[@class="box"]//div[@class="postmsg"]/descendant::text()['
            'not(ancestor::div[@class="quotebox"])]'
        )
        post_text = " ".join([
    ...
    def get_avatar(self, tag):
        avatar_block = tag.xpath(
            'div[@class="box"]//dd[@class="postavatar"]/img/@src'
        )
        if not avatar_block:
            return ""
