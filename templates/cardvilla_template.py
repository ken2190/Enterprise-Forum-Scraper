...
class CardvillaParser:
    def __init__(self, parser_name, files, output_folder, folder_path):
        # locale.setlocale(locale.LC_TIME, 'ru_RU.UTF-8')
        self.parser_name = "cardvilla.net"
    ...
    def get_title(self, tag):
        title = tag.xpath(
            '//span[contains(@class,"threadtitle")]//descendant::text()'
        )
    ...
    def get_avatar(self, tag):
        avatar_block = tag.xpath(
            './/div[contains(@class,"userinfo")]//a[@class="postuseravatar"]//img/@src'
        )
        if not avatar_block:
            return ""
